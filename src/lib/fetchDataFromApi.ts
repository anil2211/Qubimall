...
// Get the base URL from environment or fallback to window.location.origin in the browser
const baseURL =
  typeof window !== "undefined"
    ? `${window.location.origin}/api`
    : process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000/api";

// Helper to safely get token from cookie (only in the browser)
...
const fetchData = {
  get: async (url: string, params: Record<string, any> = {}) => {
    try {
      ...
      console.log("Making GET request:", { url, config });
